    ...
    public partial class ViewerPageControl : UserControl
    {
        ...
        private async Task LoadGroupsAsync()
        {
            ...
            try
            {
                ...
                cmbGroupFilter.SelectedIndex = SelectedIndex;
            }
            catch (MySqlException ex) when (ex.Message.Contains("doesn't exist") || ex.Message.Contains("Unknown database"))
            {
                // 表或数据库尚未创建，静默忽略
        ...
        private async Task LoadMessagesAsync()
        {
            try
            {
                ...
                DateTime end = dtpEndDate.Value.Date.AddDays(1).AddSeconds(-1).ToUniversalTime();
                ...
                // 将 created_at 列从 UTC 转换为本地时间
                if (dt.Columns.Contains("created_at"))
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        if (row["created_at"] != DBNull.Value)
                        {
                            DateTime utcTime = (DateTime)row["created_at"];
